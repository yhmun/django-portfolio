...
def create_sample_product_data(apps, schema_editor):
    ...
    Product(
        ...
        photo='example/store/products/mineralwater-strawberry.jpg',
    ).save()
    Product(
        id=2, name='Mineral Water Raspberry', description='Flavoured with raspberry, loaded with anti-oxidants.', price=2.00,
        photo='example/store/products/mineralwater-raspberry.jpg',
    ).save()
    Product(
        ...
        photo='example/store/products/vitamin-a.jpg',
    ).save()
    Product(
        ...
        sale_start=timezone.now(),
        sale_end=timezone.now() + timedelta(days=10),
        photo='example/store/products/vitamin-bcomplex.jpg',
    ).save()
